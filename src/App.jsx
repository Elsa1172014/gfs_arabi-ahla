...
.lh-formshell{display:grid;grid-template-columns:1fr 280px;gap:22px;max-width:900px;margin:0 auto;align-items:start}
@media(max-width:820px){.lh-formshell{grid-template-columns:1fr}}
.lh-panel{background:#fff;border-radius:16px;padding:22px;color:${T.ink};box-shadow:0 20px 50px -20px rgba(0,0,0,.55)}
      ...
      <div className="wrap lh-shell">
        {stage === "welcome" && (
          <div className="lh-hero lh-fade">
            ...
            <h1 style={{ color: "#fff", fontSize: 34, textAlign: "center", margin: "10px 0 8px" }}>مرحبًا بك في منصة GFS بالعربي أحلى</h1>
            <p style={{ color: "#E7DCE8", textAlign: "center", fontSize: 16, margin: "0 0 26px" }}>رحلة ذكية لإتقان اللغة العربية</p>
            <div style={{ textAlign: "center" }}>
              <button className="btn lh-cta" onClick={(e) => { ripple(e); setStage("pick"); }}>ابدأ الآن ←</button>
            </div>
          </div>
        )}
        ...
        {stage === "pick" && (
          <div className="lh-fade">
            <h2 style={{ color: "#fff", textAlign: "center", marginBottom: 22 }}>من أنت؟</h2>
            <div className="lh-rolegrid">
              {ROLES.filter((r) => r.k !== "a" || staffOpen).map((r) => (
            ...
            {!staffOpen && <div style={{ textAlign: "center", marginTop: 16 }}>
              <button className="btn btn-q" style={{ color: "#E7DCE8" }} onClick={() => setStaffOpen(true)}>دخول رئيس القسم</button>
            </div>}
          </div>
        )}
        ...
        {stage === "form" && (
          <div className="lh-formshell lh-fade">
            <div className="lh-panel">
              <button className="btn btn-q" style={{ marginBottom: 10, color: T.inkSoft }} onClick={() => { setStage("pick"); setErr(""); }}>→ رجوع لاختيار الدور</button>
            ...
            <aside className="lh-side">
              <svg className="lh-illustration" viewBox="0 0 200 200" aria-hidden="true">
                <circle cx="100" cy="100" r="90" fill="none" stroke="rgba(255,255,255,.25)" strokeWidth="2" />
                <path d="M100 20 L120 80 L180 80 L132 114 L150 174 L100 138 L50 174 L68 114 L20 80 L80 80 Z" fill="rgba(255,255,255,.12)" stroke="rgba(255,255,255,.35)" />
              </svg>
...
/* ==================== لوحة المعلم الكاملة ==================== */
function TeacherHome({ teacherName, courses, attempts, progress, students, onNew, onManual, onPaste, onPublish, onView, onEdit, onAssign, onArchive, onSendReport, onExport, onImportFile, onTemplate, onAddStudent, onRemoveStudent, onDuplicateCourse }) {
...
export default function App() {
  ...
  function mergeStudentRows(rows, actor) {
    ...
    rows.forEach((r) => {
      ...
      // نفس صيغة المفتاح المستعملة عند دخول الطالب بنفسه: آخر 6 أرقام فقط،
      // بصرف النظر عن طول الرقم التعريفي الكامل الذي رُفع في الملف —
      // فيتطابق الطالب مع تسجيله عند الدخول لاحقًا مهما كان شكل الرقم المرفوع.
