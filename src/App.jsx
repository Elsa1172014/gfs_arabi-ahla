...
/* ==================== لوحة المعلم الكاملة ==================== */
function TeacherHome({ teacherName, courses, attempts, progress, students, onNew, onManual, onPaste, onPublish, onView, onEdit, onAssign, onArchive, onSendReport, onExport, onImportFile, onTemplate, onAddStudent, onRemoveStudent, onEditStudent, onDuplicateCourse }) {
    ...
    <div className="wrap" style={{ paddingBottom: 60 }}>
      ...
      {tab === "s" && (<div className="card" style={{ padding: 20, overflowX: "auto" }}>
        ...
        {rows.length === 0 ? <p style={{ color: T.inkSoft }}>لم يسجّل أي طالب بعد.</p> : (
          <table className="tbl"><thead><tr><th>الطالب</th><th>الصف</th><th>البلوك</th><th>مسند</th><th>بدأ</th><th>أتقن</th><th>محاولات</th><th>المتوسط</th><th>الحالة</th><th></th><th></th><th></th></tr></thead>
            <tbody>{rows.map((r) => (<React.Fragment key={r.key}><tr>
              ...
              <td><button className="btn btn-q" onClick={() => { setEditKey(editKey === r.key ? null : r.key);
                setEditForm({ name: r.name, sid: r.key.split("-")[2] && /^\d{6}$/.test(r.key.split("-")[2]) ? r.key.split("-")[2] : "", grade: r.grade || 7, block: r.block === "؟" ? "A" : r.block, stream: r.stream, parentEmail: r.parentEmail || "" }); }}>
            ...
            {editKey === r.key && <tr><td colSpan={12}>
              <div className="grid" style={{ gridTemplateColumns: "repeat(5,1fr)", padding: 10, background: T.paper }}>
                ...
                <div><label className="lbl">آخر 6 أرقام</label><input className="inp mono" value={editForm.sid} maxLength={6} onChange={(e) => setEditForm({ ...editForm, sid: e.target.value.replace(/\D/g, "") })} /></div>
            ...
            {expandStudent === r.key && <tr><td colSpan={12}>
...
export default function App() {
  ...
  function mapColumns(headerRow) {
    ...
    (headerRow || []).forEach((cell, i) => {
      ...
      Object.entries(HEADER_MAP).forEach(([field, names]) => {
        if (idx[field] === undefined && names.some((n) => n.toLowerCase() === c)) idx[field] = i;
      });
    });
  ...
  function mergeStudentRows(rows, colIdx, actor) {
    ...
    rows.forEach((r) => {
      ...
      // معرّفًا مؤقتًا فريدًا إن لم يوجد رقم مدرسي صالح؛ يُعلَّم السجل
      // «يحتاج مراجعة» ليكمله المعلم لاحقًا من شاشة «طلابي».
      const needsReview = !/^\d{4,8}$/.test(sidDigits) || !g || block === "؟";
      const sid = /^\d{4,8}$/.test(sidDigits) ? sidDigits.slice(-6) : `TMP${(Date.now() + placeholderSeq++).toString().slice(-6)}`;
      const key = `${g || 0}-${block}-${sid}`;
      if (existingKeys.has(key)) return dups.push(key);
      added.push({ key, name, grade: g, block, stream: streamRaw.toUpperCase() === "B" ? "B" : "A", parentEmail: parentEmailRaw, needsReview });
  ...
  const importStudentsCSV = (txt) => {
    ...
    const hasHeader = rows.length && isHeaderRow(rows[0]);
    const colIdx = mapColumns(hasHeader ? rows[0] : null);
    const filtered = hasHeader ? rows.slice(1) : rows;
    const r = mergeStudentRows(filtered, colIdx, "رئيس القسم");
  ...
  const importStudentsFile = (file, setMsg) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const colIdx = mapColumns(hasHeader ? rows[0] : null);
        const filtered = hasHeader ? rows.slice(1) : rows;
        const r = mergeStudentRows(filtered, colIdx, user.name);
  ...
  const editStudent = (oldKey, fields, actor) => {
    ...
    const sidDigits = (fields.sid || "").replace(/\D/g, "");
    ...
    const rec = { key: newKey, name: (fields.name || existing.name).trim(), grade: g, block, stream: fields.stream || existing.stream,
      parentEmail: fields.parentEmail !== undefined ? fields.parentEmail : existing.parentEmail, needsReview };
