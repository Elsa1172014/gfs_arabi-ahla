...
const DEFAULT_BLOCKS = ["A-D", "H-N", "K-P", "E-I", "F-J"];
...
const C1 = {
  ...
  blocks: ["A-D", "H-N"], students: [], status: "published", q: 25, teacher: "معلم اللغة العربية",
...
const C2 = {
  ...
  blocks: ["A-D"], students: [], status: "published", q: 25, teacher: "معلم اللغة العربية",
...
const C3 = {
  ...
  blocks: ["A-D", "H-N", "K-P"], students: [], status: "published", q: 25, teacher: "معلم اللغة العربية",
...
/* ============================ الدخول ============================ */
function Login({ onStudent, onTeacher, onAdmin, onParent, codes, students }) {
  const [tab, setTab] = useState("s");
  const [staffOpen, setStaffOpen] = useState(false);
  const [f, setF] = useState({ name: "", sid: "", grade: 7, block: "A-D", stream: "A", email: "" });
  ...
  const go = () => {
    ...
    // مطابقة صارمة ببيانات القائمة المستوردة فعليًّا داخل المنصة — الاسم
    // والرقم والصف والبلوك يجب أن تطابق سجلًّا موجودًا مسبقًا، وإلا يُرفَض
    // الدخول تمامًا بدل إنشاء طالب جديد تلقائيًّا كما كان سابقًا.
    ...
    setErr(""); onStudent({ ...f, name: existing.name, role: "student", key });
  };
  return (
    <div className="gfs"><style>{CSS}</style>
      <div className="topbar" style={{ color: "#fff", padding: "44px 0 62px" }}><div className="wrap topbar-inner">
        <div className="brand-row" style={{ marginBottom: 22 }}>
          ...
          <div>
            <div className="brand-tagline">قسم اللغة العربية — ننمو · ننجح · نزدهر</div>
...
function CourseBuilder({ teacherName, initial, onSave, onCancel }) {
  const [c, setC] = useState(() => initial || {
    ...
    blocks: ["A-D"], students: [], status: "draft", q: 25, teacher: teacherName,
...
function Generator({ teacherName, onSave, onCancel }) {
  const [f, setF] = useState({ skill: "", domain: "SP", grade: 7, stream: "A", blocks: ["A-D"] });
...
function PasteReorganizer({ teacherName, onSave, onCancel }) {
  const [f, setF] = useState({ domain: "SP", grade: 7, stream: "A", blocks: ["A-D"] });
...
/* ==================== لوحة المعلم الكاملة ==================== */
function TeacherHome({ teacherName, courses, attempts, progress, students, onNew, onManual, onPaste, onPublish, onView, onEdit, onAssign, onArchive, onSendReport, onExport, onImportFile, onTemplate, onAddStudent, onRemoveStudent }) {
  const [tab, setTab] = useState("d");
  const fileRef = useRef(null);
  const [importMsg, setImportMsg] = useState("");
  const [addOpen, setAddOpen] = useState(false);
  const [addForm, setAddForm] = useState({ name: "", sid: "", grade: 7, block: "A-D", stream: "A" });
...
function AdminHome({ courses, students, attempts, progress, teachers, blocksAdmin, audit, codes, onUpdateCodes, orgEmail, onUpdateOrgEmail, onSetTeacherEmail, onToggleTeacher, onAddBlock, onRemoveBlock,
  ...
  return (
    <div className="wrap" style={{ paddingBottom: 60 }}>
      <div className="tabs">
        {[["o", "نظرة عامة"], ["cmp", "مقارنات"], ["an", "تحليل صف أو طالب"], ["u", "المستخدمون والصلاحيات"], ["b", "الصفوف والبلوكات"], ["c", "جميع الكورسات"], ["log", "سجل النشاط"], ["io", "استيراد وتصدير"], ["sec", "الأمان"]].map(([k, l]) => (
          <button key={k} className="tabbtn" onClick={() => setTab(k)} style={{ background: tab === k ? T.ink : T.paper, color: tab === k ? "#fff" : T.inkSoft }}>{l}</button>))}
      </div>
...
export default function App() {
  ...
  const downloadStudentTemplate = () => {
    downloadXLSX("قائمة_الطلاب_نموذج.xlsx", [
      ...
      ["سالم أحمد الكعبي", "123456", "7", "A-D", "A", "parent1@example.com"],
      ["ليان محمد النعيمي", "654321", "7", "H-N", "A", ""],
    ]);
  };
